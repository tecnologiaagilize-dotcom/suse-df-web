...
const PATENTES = [
    'Soldado PM', 'Cabo PM', '3º Sargento PM', '2º Sargento PM', '1º Sargento PM', 'Subtenente PM',
    'Aspirante a Oficial PM', '2º Tenente PM', '1º Tenente PM', 'Capitão PM', 'Major PM', 
    'Tenente-Coronel PM', 'Coronel PM',
    'Agente de Polícia', 'Escrivão de Polícia', 'Papiloscopista', 'Agente Policial de Custódia', 
    'Delegado de Polícia', 'Perito Criminal', 'Perito Médico-Legista'
];

const UNIDADES = [
    '1º BPM (Asa Sul)', '2º BPM (Taguatinga)', '3º BPM (Asa Norte)', '4º BPM (Guará)', '5º BPM (Lago Sul)',
    '6º BPM (Esplanada)', '7º BPM (Sudoeste)', '8º BPM (Ceilândia)', '9º BPM (Gama)', '10º BPM (Ceilândia)',
    '1ª DP (Asa Sul)', '2ª DP (Asa Norte)', '3ª DP (Cruzeiro)', '4ª DP (Guará)', '5ª DP (Área Central)',
    '6ª DP (Paranoá)', '12ª DP (Taguatinga Centro)', '21ª DP (Taguatinga Sul)', '35ª DP (Sobradinho II)'
];
...
export default function Dashboard() {
  ...
  // --- NOVOS ESTADOS PARA VALIDAÇÃO POLICIAL (MERGE) ---
  const [showValidationModal, setShowValidationModal] = useState(false);
  const [selectedAlertForValidation, setSelectedAlertForValidation] = useState(null);
  ...
  // --- NOVA FUNÇÃO DE VALIDAÇÃO (MERGE) ---
  const handleOpenValidationModal = (alert) => {
      setSelectedAlertForValidation(alert);
      setValidationData({
          token: '',
          rank: '',
          name: '',
  ...
  const handleValidateToken = async (e) => {
    e.preventDefault();
    if (!validationData.token || !validationData.rank || !validationData.name || !validationData.matricula || !validationData.battalion) {
        setValidationError("Preencha todos os campos obrigatórios.");
        return;
    }
    
    if (!selectedAlertForValidation) {
        setValidationError("Nenhuma ocorrência selecionada.");
        return;
    }
    ...
    try {
      const { data, error } = await supabase
        .rpc('validate_termination_token', { 
            p_alert_id: selectedAlertForValidation.id,
            p_token_input: validationData.token,
            p_rank: validationData.rank,
            p_name: validationData.name,
            p_matricula: validationData.matricula,
            p_phone: validationData.phone || 'N/A',
            p_battalion: validationData.battalion
        });
      ...
      if (data && data.success) {
          ...
          setValidationData({ token: '', rank: '', name: '', matricula: '', phone: '', battalion: '' });
          
          // Fechar janela se estiver aberta
          closeWindow(selectedAlertForValidation.id);
          
          fetchAlerts();
          setSelectedAlertForValidation(null);
      } else {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case 'waiting_police_validation': return 'bg-orange-100 text-orange-800 border-2 border-orange-500 animate-pulse'; // Novo status
  ...
  const getStatusLabel = (status) => {
      switch (status) {
          case 'active': return 'EMERGÊNCIA';
          case 'investigating': return 'EM INVESTIGAÇÃO';
          case 'waiting_police_validation': return 'AGUARDANDO VALIDAÇÃO';
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
              ...
              <div className="flex gap-4">
                  {activeWindows.map(window => (
                      <div key={window.id} className="inline-block w-[800px] h-[400px] bg-white rounded-lg shadow-xl overflow-hidden border border-gray-300 flex flex-col relative shrink-0">
                          ...
                          <div className={`text-white px-4 py-2 flex justify-between items-center h-12 ${window.status === 'waiting_police_validation' ? 'bg-orange-600' : 'bg-gray-800'}`}>
                              <div className="flex items-center gap-4">
                                  <span className={`font-bold uppercase tracking-wider flex items-center gap-1 ${window.status === 'waiting_police_validation' ? 'text-white' : 'text-red-400'}`}>
                                    <AlertTriangle size={16} /> {getStatusLabel(window.status)}
                                  </span>
                                  ...
                                  <StaticDuration 
                                    ...
                                    label="Espera"
                                    icon={Timer}
                                    colorClass={window.status === 'waiting_police_validation' ? 'text-white' : 'text-yellow-400'}
                                  />
                                  ...
                                  <ProgressiveTimer 
                                    ...
                                    label="Duração"
                                    icon={Clock}
                                    colorClass={window.status === 'waiting_police_validation' ? 'text-white' : 'text-green-400'}
                                  />

                                  {window.status === 'waiting_police_validation' ? (
                                      <button 
                                          onClick={() => handleOpenValidationModal(window)}
                                          className="flex items-center gap-1 bg-white text-orange-600 hover:bg-gray-100 text-xs px-2 py-1 rounded transition-colors ml-2 font-bold animate-pulse"
                                          title="Validar Token"
                                      >
                                          <Shield size={14} /> VALIDAR TOKEN
                                      </button>
                                  ) : (
                                      <button 
                                    ...
                                    <div className="absolute bottom-0 left-0 right-0 bg-white/90 backdrop-blur-sm p-2 px-4 border-t border-gray-200 flex justify-between items-center">
                                        <div className="flex items-center gap-4">
                                            <div className="flex items-center gap-2 text-xs">
                                                <span className={`w-2 h-2 rounded-full ${window.status === 'waiting_police_validation' ? 'bg-orange-500' : 'bg-red-500'} animate-pulse`}></span>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div className="bg-white w-full max-w-4xl max-h-[90vh] flex flex-col rounded-lg shadow-2xl overflow-hidden animate-fade-in">
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    {/* Conteúdo do Modal de Detalhes (Mantido igual ao backup) */}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-6">
                            ...
                            {/* Visualização da Foto de Encerramento (se houver) */}
                            {showDetailsModal.termination_photo_url && (
                                <div>
                                    <h4 className="text-sm font-bold text-orange-600 uppercase tracking-wider mb-3 flex items-center gap-2">
                                        <Eye size={16} /> Foto de Segurança (Encerramento)
                                    </h4>
                                    <a href={showDetailsModal.termination_photo_url} target="_blank" rel="noopener noreferrer" className="block relative group rounded-lg overflow-hidden border border-orange-200">
                                        <img 
                                            src={showDetailsModal.termination_photo_url} 
                                            alt="Foto de Encerramento" 
                                            className="w-full h-48 object-cover"
                                        />
                                        <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                            <span className="text-white font-bold flex items-center gap-2"><ExternalLink size={16} /> Ampliar</span>
                                        </div>
                                    </a>
                                </div>
                            )}
      ...
      {/* --- NOVO MODAL DE VALIDAÇÃO POLICIAL (MERGE) --- */}
      {showValidationModal && (
        <div className="fixed inset-0 bg-black/60 z-[80] flex items-center justify-center p-4">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
                <div className="bg-blue-600 p-4 flex justify-between items-center text-white shrink-0">
                    <h3 className="font-bold flex items-center gap-2">
                        <Shield size={20} /> Validação Policial de Segurança
                    </h3>
                ...
                <form onSubmit={handleValidateToken} className="p-6 overflow-y-auto">
                    <div className="bg-blue-50 border-l-4 border-blue-500 p-4 mb-6 text-sm text-blue-800">
                        <p className="font-bold mb-1">Procedimento de Validação:</p>
                        <ul className="list-disc list-inside space-y-1">
                            <li>O motorista deve estar fisicamente presente em uma unidade policial.</li>
                            <li>Colete os dados do agente que está atendendo o motorista.</li>
                            <li>Insira o Token de 6 dígitos gerado no app do motorista.</li>
                        </ul>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                        ...
                        <div className="md:col-span-2">
                            <h4 className="font-bold text-gray-700 border-b pb-2 mb-3 flex items-center gap-2">
                                <User size={16} /> Dados do Agente Responsável
                            </h4>
                        </div>

                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Patente / Cargo</label>
                            <select
                                required
                                className="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 border p-2"
                                value={validationData.rank}
                                onChange={(e) => setValidationData({...validationData, rank: e.target.value})}
                            >
                                <option value="">Selecione...</option>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                required
                                className="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 border p-2"
                                value={validationData.matricula}
                                onChange={(e) => setValidationData({...validationData, matricula: e.target.value})}
                                placeholder="Ex: 123.456-7"
                            />
                        </div>

                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Telefone (Funcional/Pessoal)</label>
                            <input
                                type="text"
                                className="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 border p-2"
                        ...
                        <div className="md:col-span-2">
                            <label className="block text-sm font-medium text-gray-700 mb-1">Batalhão / Delegacia</label>
                            <select
                                required
                                className="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 border p-2"
                                value={validationData.battalion}
                                onChange={(e) => setValidationData({...validationData, battalion: e.target.value})}
                            >
                    ...
                    <div className="border-t pt-6">
                        <label className="block text-sm font-bold text-gray-700 mb-2 text-center">
                            TOKEN DE SEGURANÇA (Fornecido pelo Motorista)
                        </label>
                        <div className="flex justify-center">
                            <input
                                ...
                                maxLength={6}
                                required
                                value={validationData.token}
                                onChange={(e) => setValidationData({...validationData, token: e.target.value.toUpperCase()})}
                                placeholder="AB12CD"
                                className="w-48 text-center text-4xl font-mono tracking-widest border-2 border-gray-300 rounded-lg p-3 focus:border-blue-500 focus:ring-blue-500 uppercase"
                            />
                        </div>
                        {validationError && (
                            <p className="text-red-600 text-sm mt-3 flex items-center gap-1 justify-center font-bold animate-pulse">
                    ...
                    <div className="mt-8 flex gap-3">
                        ...
                        <button
                            type="submit"
                            disabled={isValidating || validationData.token.length < 6}
                            className="flex-1 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg shadow transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex justify-center items-center gap-2"
                        >
                            {isValidating ? 'Validando...' : 'Confirmar e Encerrar'}
                        </button>
                    </div>
                </form>
            </div>
                  ...
                  {alerts.map((alert) => (
                    <tr key={alert.id} className={`hover:bg-gray-50 ${activeWindows.find(w => w.id === alert.id) ? 'bg-blue-50' : ''}`}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        ...
                        {/* Botão de Validação na Tabela também */}
                        {alert.status === 'waiting_police_validation' && (
                             <button 
                                onClick={() => handleOpenValidationModal(alert)}
                                className="mt-2 text-orange-600 hover:text-orange-800 text-xs font-bold flex items-center gap-1"
                             >
                                 <Shield size={12} /> Validar Token
                             </button>
                        )}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
