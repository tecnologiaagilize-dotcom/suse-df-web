-- CORREÇÃO MESTRA (Rode tudo de uma vez)

-- 1. Relaxar Constraints da Tabela Users (Permitir cadastro incompleto)
...
ALTER TABLE public.users ALTER COLUMN emergency_contacts DROP NOT NULL;
ALTER TABLE public.users ALTER COLUMN car_brand DROP NOT NULL;
ALTER TABLE public.users ALTER COLUMN car_model DROP NOT NULL;
ALTER TABLE public.users ALTER COLUMN car_plate DROP NOT NULL;
ALTER TABLE public.users ALTER COLUMN car_color DROP NOT NULL;
...
-- 2. Garantir Políticas de RLS para Users
...
-- 3. Criar Buckets e Permissões (termination-evidence)
INSERT INTO storage.buckets (id, name, public) VALUES ('termination-evidence', 'termination-evidence', true) ON CONFLICT (id) DO NOTHING;
INSERT INTO storage.buckets (id, name, public) VALUES ('avatars', 'avatars', true) ON CONFLICT (id) DO NOTHING;

-- Permissões ABERTAS para debug (Cuidado em produção, mas necessário agora)
DROP POLICY IF EXISTS "Public Upload Evidence" ON storage.objects;
CREATE POLICY "Public Upload Evidence" ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id = 'termination-evidence');

DROP POLICY IF EXISTS "Public View Evidence" ON storage.objects;
CREATE POLICY "Public View Evidence" ON storage.objects FOR SELECT TO authenticated, anon USING (bucket_id = 'termination-evidence');

-- 4. Garantir que Motorista possa ATUALIZAR o status do seu alerta
